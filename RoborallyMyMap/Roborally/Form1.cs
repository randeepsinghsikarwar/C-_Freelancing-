...
namespace Roborally
{
    ...
    class Board
    {
        ...
        public Board(int w, int h, int s, PictureBox p)
        {
            ...
            robot = new Robot(this, new Point(0,0));
            addFeature(robot);

            for(int i = 1; i < Width - 2; i++)
            {
                Conveyor c = new Conveyor(this, new Point(i, 0), Direction.Right);
                addFeature(c);
            }

            Conveyor c3 = new Conveyor(this, new Point(Width-1, 0), Direction.Down, Rotation.clockwise);
            addFeature(c3);

            Pit pit = new Pit(this, new Point(4,1), Direction.Left);
            addFeature(pit);
           

            Conveyor c4 = new Conveyor(this, new Point(3, 1), Direction.Right, Rotation.clockwise);
            addFeature(c4);

           
            Wall w1 = new Wall(this, new Point(0,1), Direction.Down);
            addFeature(w1);
           
            Wall w3 = new Wall(this, new Point(0,1), Direction.Right);
            addFeature(w3);

            Conveyor c2 = new Conveyor(this, new Point(Width-1, 1), Direction.Down);
            addFeature(c2);
            Conveyor c5 = new Conveyor(this, new Point(Width-1, 2), Direction.Left, Rotation.clockwise);
            addFeature(c5);

            Winner win = new Winner(this, new Point(2, 2));
            addFeature(win);

            for(int i = 0; i < Width; i++)
            {
                Wall wall = new Wall(this, new Point(i, 2), Direction.Down);
                addFeature(wall);
            }
